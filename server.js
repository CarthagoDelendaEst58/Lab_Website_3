...
//   db.any(query)
//     .then(function (rows) {
//       console.log(rows[0].visitor_name);
...
app.get('/team_stats', function(req, res) {
  var query = 'SELECT * FROM football_games;';
  var query1 = 'SELECT COUNT(*) AS ct FROM football_games WHERE home_score > visitor_score;';
  var query2 = 'SELECT COUNT(*) AS ct FROM football_games WHERE home_score < visitor_score;';
  db.task('get-everything', task => {
      return task.batch([
          task.any(query),
          task.any(query1),
...
app.get('/player_info/post', function(req, res) {
  ...
  var query = "SELECT * FROM football_players WHERE id = " + player_choice + ";";
  var query2 = "SELECT COUNT(*) AS ct FROM football_games WHERE " + player_choice + " = ANY (players);";
  var query3 = "SELECT id,name FROM football_players;";
  db.task('get-everything', task => {
      return task.batch([
          task.any(query),
          task.any(query2),
    ...
    .then(function(data){
      ...
      res.render('pages/player_info',{
        my_title: 'Player Info',
        player_stats: data[0][0],
        games_played: data[1][0],
        data: data[2]
      })
    })
    .catch(error => {
